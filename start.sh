...
# Export mode: copy data out of the running container
if [[ "$EXPORT" == true ]]; then
  if ! docker ps --format '{{.Names}}' | grep -qx "${CONTAINER_NAME}"; then
    echo "ERROR: no running container named ${CONTAINER_NAME}"
    exit 1
  fi
  echo "Exporting container data to ${APP_DIR}/data-export ..."
  mkdir -p "${APP_DIR}/data-export"
  docker cp "${CONTAINER_NAME}:/app/data/." "${APP_DIR}/data-export/"
...
echo "Data lives inside the container; it will be lost when the container is removed."
echo "Run './start.sh --export' to copy data back to the host."
